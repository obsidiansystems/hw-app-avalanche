...
test("avalanche.getPublicKey", async () => {
  ...
  const result = await avalanche.getWalletExtendedPublicKey("m/44'/9000'/0'/1/0");
});

test("avalanche.getAppConfiguration", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
    => 8000000000
    <= 0001009000
  ...
  const avalanche = new Avalanche(transport);
  var configuration = await avalanche.getAppConfiguration();
...
test("avalanche.getWalletId", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
    => 8001000000
